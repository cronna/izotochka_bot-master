...
async def input_mail_handler(message: Message, state: FSMContext):
    ...
    if re.fullmatch(regex, message.text) is None:
        return await message.edit_text('❌ Ошибка! Проверьте адрес электронной почты')
    email = message.text
   
    await state.update_data(email=email)
   
    await state.set_state(Registration.input_number)
    await message.edit_text(f'''✅ Укажите Ваш номер телефона в формате: +79007773344''')
...
async def input_number_handler(message: Message, state: FSMContext):
    ...
    try:
        ...
        return await message.edit_text('❌ Ошибка! Введён неверный номер телефона, попробуйте ещё раз')
    ...
    await state.clear()
    await message.edit_text('''Отлично, теперь вы можете оплачивать услуги нашей студии.
